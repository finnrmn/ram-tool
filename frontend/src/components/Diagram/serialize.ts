import type { DiagramEdge, DiagramNode, RootKind } from "./types";
import { isComponentData, isKofNData } from "./types";
import type { Scenario, Component as ScenarioComponent, Structure } from "../../types";
...
type NodeLookup = Map<string, DiagramNode>;

const buildNodeLookup = (nodes: DiagramNode[]): NodeLookup => {
  const lookup: NodeLookup = new Map();
  nodes.forEach((node) => lookup.set(node.id, node));
  return lookup;
};

const sortComponentsByPosition = (components: DiagramNode[], axis: "x" | "y") => {
...
export const serializeDiagram = (
  ...
  let orderedComponents: DiagramNode[] = [];
  ...
  if (rootKind === "series") {
    ...
    const orderedByEdge = childEdges
      ...
      .filter((node): node is DiagramNode => Boolean(node && isComponentData(node.data)));
  ...
  const structure: Structure = { kind: rootKind };
  ...
  const components: ScenarioComponent[] = orderedComponents.map((node, index) => {
    const data = node.data;
    if (!isComponentData(data)) {
      throw new Error("Komponentendaten fehlen.");
    }
    const distribution: ScenarioComponent["distribution"] = { type: "exponential" };
    ...
    const component: ScenarioComponent = {
      id: node.id,
